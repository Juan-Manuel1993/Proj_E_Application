...
$typesR_list = getRelationsTypesAUTOC($data);
...
<body>
  ...
  $(document).ready(function() {
    ...
    const regex = /r_[^\n'\s]+/g;
    var typesR_list = <?php echo json_encode($typesR_list); ?>;
    var typesR_tab = typesR_list.match(regex);
    var unique = [];

    $.each(typesR_tab, function(i, el){
    ...
    $.each(document. querySelectorAll ("input.form-control.form-control-sm"), function(i, el){
      autocomplete(el, unique.sort());
    });
...
console.log(json);
    ...
    $.each(json.REntrantes, function(key, val) {
      var entrie = getEntrie(json.entries, val.node);
      if(entrie != null){
        dtableEntrante.row.add( [ val.w ,entrie.name, getRelationType(json.RTypes,val.type).trname ] ).node().id = val.node;
      }
    });
...
<div id="conteneur">
  <div>
    <h2>Mot de la recherche : <h1 align="center" ><?php echo $word; ?></h1> </h2>
    <h3>Définition : </h3><br>
    <?php echo $news['definition'] ?>
    <br>
    <h3>Raffinement : </h3><br>
