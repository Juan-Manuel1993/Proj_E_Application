...
*/
	...
	function getNodesTypes($data){
		$string = explode("// les types de noeuds (Nodes Types) : nt;ntid;'ntname'",$data);
		$string = explode("// les noeuds/termes (Entries) : e;eid;'name';type;w;'formated name'",$string[1]);

		$regexAll = '/([\n]*nt;\d+;[^\n]+)/u';
		$regexSingle = '/nt;(?P<ntid>\d+);(?P<ntname>[^.]+)/u';
	...
	function getInComingRelations($data){
		...
		$regexAll = '/([\n]*r;\d+;\d+;\d+;\d+;\d+)/u';
		$regexSingle = '/r;(?P<rid>\d+);(?P<node1>\d+);(?P<node2>\d+);(?P<type>\d+);(?P<w>\d+)/u';

		return getToken($string[0],$regexAll,$regexSingle);
	}

	function getOutGoingRelations($data){
		$string = explode("// les relations sortantes : r;rid;node1;node2;type;w",$data);
	...
	function getNodeType($nodestypes,$ntid){
		foreach ($nodestypes as $key => $subtab) {
			if($subtab['ntid'] == $ntid){
				return $subtab;
			}
		}
	...
	function getRelationType($relationstypes,$rtid){
		foreach ($relationstypes as $key => $subtab) {
			if($subtab['rtid'] == $rtid){
				return $subtab;
			}
		}
	...
	$entries = array_sort(getEntries($data), 'w', SORT_DESC);
	print_r($entries);
	//////////////////////
	/*
